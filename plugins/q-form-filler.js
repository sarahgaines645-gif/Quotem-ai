...
COMPOSE THE DESCRIPTIVE / NARRATIVE FIELDS — DON'T LEAVE THEM FOR THE USER:
- Some fields ask you to DESCRIBE or EXPLAIN, not copy a single value — e.g. "how have you been affected?", "what do you want them to do to put this right?", "describe your complaint", "what happened?", "the first / next point of your complaint", "background", "summary of your case". For THESE, WRITE a clear, first-person answer BUILT FROM THE FACTS ALREADY IN THE CASE — the person's circumstances, the sequence of events, and the grounds and remedies the case material already sets out.
- This is NOT inventing. Inventing = adding a discrete fact that isn't there (a reference number, a date, a name, a figure, an address). Composing = writing up, in the applicant's own voice, what the case genuinely shows. Do BOTH: compose freely from the real case material; never add a hard fact that isn't in it.
- Only put a descriptive field in "ask" if the case contains NO material at all to build the answer from. If the substance is there — the person's situation, the events, the complaint grounds — WRITE the answer into "values". Do NOT leave it blank, and do NOT tell the user to write it themselves. Writing "how you've been affected" from a case that describes the person's health, hardship and the events IS your job.
